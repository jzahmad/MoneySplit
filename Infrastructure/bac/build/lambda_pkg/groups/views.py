...
def cognito_user_exists(email: str) -> bool:
    ...
    logger.info("Checking if Cognito user exists", extra={"email": email})

    pool_id = os.environ.get("COGNITO_USER_POOL_ID")
    if not pool_id:
        logger.warning(
            "COGNITO_USER_POOL_ID not set; skipping Cognito lookup",
            extra={"email": email},
        )
        return False
    ...
    try:
        logger.info(
            ...
            extra={"user_pool_id": pool_id, "email": email},
        )
        ...
        logger.info("Cognito user FOUND", extra={"email": email})
        return True

    except client.exceptions.UserNotFoundException:
        logger.info("Cognito user NOT found", extra={"email": email})
        return False

    except Exception as e:
        # This is critical for debugging 503s
        logger.exception(
            "Error while checking Cognito user existence",
            extra={"email": email, "user_pool_id": pool_id},
        )
        raise
